...
현재 버전: **1.7** · [릴리즈](https://github.com/jimyeong-han/go-valuedex/releases/tag/v1.7) · [변경 기록](CHANGELOG.md)

## 제공 기능

- 1,188개 평가 폼의 한국어·영문명·도감 번호 검색과 타입/`세대 (지역)`/메가/다이맥스 필터
- 일반·지역·전투 폼을 독립적으로 선택하고 공유할 수 있는 폼별 URL
- 5·10 구분선이 있는 공격·방어·체력 `0–15` 슬라이더, 동기화된 숫자 직접 입력, 강화 레벨에 따른 현재 예상 CP 실시간 표시
...
- 보스·메가/원시 폼, 난이도, 보스 기술, 날씨, HP·CPM·타이머를 지정하는 레이드·맥스 보스별 PvE 분석
- 공격수 순위를 3마리씩 묶어 보여주고 펼치면 개별 기술·DPS·TDO·처치시간을 확인하는 추천 트리오(같은 원본·Mega 중복 방지, Mega 최대 1마리)
...
- 현재 폼·상태·IV·레벨과 선택한 노말·차지 기술, Max 자격·특훈 계획을 한 번에 저장하는 로컬 보유함
...
- [Niantic 맥스배틀 도움말](https://niantic.helpshift.com/hc/en/6-pokemon-go/faq/4798-max-battles-and-catching-max-pokemon-1729887131/?hl=ko&p=web): 맥스 포켓몬 3마리 선택과 맥스미터·맥스 기술의 공식 규칙
...
- `다이맥스 (일반 기술 근사)` 프리셋은 맥스 참가 가능 종 또는 보유 맥스 개체만 추려 일반 기술 구간의 레이드식 순위를 비교합니다. 맥스미터, 3턴 동안의 맥스어택·맥스가드·맥스스피릿, 실제 맥스 보스별 HP·배율은 반영하지 않으며, HP·시간·CPM은 사용자가 보정하는 5성 레이드 초깃값입니다.
- 추천 트리오는 계산된 개별 공격수 순위를 세 개씩 묶은 탐색 UI입니다. 한 묶음에는 같은 도감 원본 또는 같은 보유 개체의 일반·Mega 대안을 중복하지 않고, 공식 활성 제한에 맞춰 Mega를 최대 한 마리만 넣습니다. 교체 순서·기절·역할 분담을 계산한 완전한 3마리 파티 시뮬레이션은 아닙니다.
