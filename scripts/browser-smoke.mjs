...
const checks = [
  ...
  ['attack slider', await evaluate("(()=>{const e=document.querySelector('#iv-attack');e.value=15;e.dispatchEvent(new Event('input',{bubbles:true}));return document.querySelector('#appraisalPercent').textContent.includes('35/45')})()")],
  ...
  ['Purified direct input does not apply +2 again', await evaluate("(()=>{state.ivs={attack:14,defense:13,stamina:15};state.level=20;selectPokemon('1:normal',false);document.querySelector('[data-mode=great]').click();document.querySelector('[data-condition=purified]').click();return state.condition==='purified'&&document.querySelector('#appraisalPercent').textContent.includes('42/45')&&document.querySelector('#ivResult .explanation').textContent.includes('+2')&&document.querySelector('#scenarioCompare').hidden})()")],
  ...
  ['evolution navigation preserves selected league', await evaluate("(async()=>{selectPokemon('1:normal',false);document.querySelector('[data-mode=ultra]').click();document.querySelector('#projectionGrid [data-select-key=\"3:normal\"]').click();await new Promise(resolve=>setTimeout(resolve,80));return state.selected.speciesKey==='3:normal'&&state.mode==='ultra'&&document.querySelector('[data-mode=ultra]').classList.contains('active')&&document.querySelector('#ivResult h4').textContent.includes('하이퍼리그')})()")],
];
